...
const parseAndGenerateFile = function(configPath, source, minimize = false) {
    return new Promise((resolve, reject) => {
        ...
        if (minimize) {
            const pipeline = postcss(cssnano())
                .process(cssString)
                .then(result => {
                    const { css = '' } = result;
...
const atomicLoader = function(source, map) {
    ...
    const tasks = configPaths.map(configPath => {
        return parseAndGenerateFile(configPath, source, minimize);
    });
